...
if ($_SERVER['REQUEST_METHOD'] == 'OPTIONS') {
    ...
    if (isset($_SERVER['HTTP_ACCESS_CONTROL_REQUEST_HEADERS']))
        header("Access-Control-Allow-Headers: {<?php
// CORS Headers for API access
if (isset($_SERVER['HTTP_ORIGIN'])) {
    header("Access-Control-Allow-Origin: {$_SERVER['HTTP_ORIGIN']}");
    header('Access-Control-Allow-Credentials: true');
...
SERVER['HTTP_ACCESS_CONTROL_REQUEST_HEADERS']}, Authorization");
    exit(0);
}
...
SERVER['HTTP_AUTHORIZATION']) && is_api_request()) {
    $token = trim(str_replace('Bearer', '', <?php
// CORS Headers for API access
if (isset($_SERVER['HTTP_ORIGIN'])) {
    header("Access-Control-Allow-Origin: {$_SERVER['HTTP_ORIGIN']}");
    header('Access-Control-Allow-Credentials: true');
...
SESSION['user_id'] = $res->fetch_assoc()['user_id'];
        }
    }
}
